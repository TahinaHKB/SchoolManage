...
<?php
	$header = ob_get_clean();
	$req2 = $bdd->prepare("SELECT * FROM professeur WHERE Id=:e");
	$req2->execute(array(
		"e" => $me['Id']
	));
	$prof = $req2->fetch();
	...
	$rech = $bdd->prepare("SELECT * FROM enseigne WHERE Professeur=:p");
	$rech->execute(array(
		"p" => $me['Id']
	));
	$data = $rech->fetchAll();
?>
	<div id="pro">
    	<form action="modele/mettreAjourProf.php" enctype="multipart/form-data" method="POST">
				<label for="renums"><img src="image/prof/<?= $prof['photo'] ?>" width="100" loading="lazy"></label>
...
<div class="bd-example">
	...
	<?php
		foreach ($data as $key => $value) {
			$rech = $bdd->prepare("SELECT * FROM matiere WHERE Id=:p");
			$rech->execute(array(
				"p" => $value['Matiere']
			));
			$m = $rech->fetch();
			$rech = $bdd->prepare("SELECT * FROM classe WHERE Id=:p");
			$rech->execute(array(
				"p" => $value['Classe']
			));
			$c = $rech->fetch();
			$rech = $bdd->prepare("SELECT * FROM correspondance WHERE Classe=:c AND Matiere=:m");
			$rech->execute(array(
				"c" => $c['Id'],
				"m" => $m['Id']
			));
