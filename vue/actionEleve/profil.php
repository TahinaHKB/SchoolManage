...
<?php
	$header = ob_get_clean();
	$req2 = $bdd->prepare("SELECT * FROM eleve WHERE Id=:e");
	$req2->execute(array(
		"e" => $me['Id']
	));
	$prof = $req2->fetch();
	...
	$rech = $bdd->prepare("SELECT * FROM Classe WHERE Id=:i");
	$rech->execute(array(
		"i" => $me['Classe']
	));
	$cl = $rech->fetch();
	$rech = $bdd->prepare("SELECT * FROM professeur WHERE Id=:i");
	$rech->execute(array(
		"i" => $cl['Titulaire']
	));
	$t = $rech->fetch();
?>
	<div id="pro">
    	<form action="modele/mettreAjourEleve.php" enctype="multipart/form-data" method="POST">
				<label for="renums"><img src="image/eleve/<?= $prof['Photo'] ?>" width="100" loading="lazy"></label>
...
<div class="bd-example">
        <table class="table table-hover" id="table">
          ...
          <tr>
            <th scope="row">Votre moyenne generale : </th>
            <td><?=$_SESSION['moyenne']?></td>
          </tr>
          <tr>
          	<th scope="row">Votre classe : </th>
          	<td><?=$cl['Nom'].$cl['Identifiant']?> <?php if(isset($t['Appelation'])){?>titularisé par <?=$t['Appelation']?><?php } ?></td>
          </tr>
          <tr>
		  ...
		  <tr>
		  	<form method="POST" action="modele/mettreAjourEleve.php">
				...
				<td><input type="mail" name="renum" id="renum" value="<?= $prof['Mail'] ?>">
				<input type="hidden" name="Id" value="<?=$prof['Id']?>">
				<input type="hidden" name="perso" value="<?=$prof['Appelation']?>">
